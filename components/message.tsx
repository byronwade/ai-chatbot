...
const PurePreviewMessage = ({
  ...
  return (
		<AnimatePresence>
			<motion.div className="w-full mx-auto max-w-3xl px-4 group/message" initial={{ y: 5, opacity: 0 }} animate={{ y: 0, opacity: 1 }} data-role={message.role} suppressHydrationWarning>
				<div
							...
							<div className="flex flex-col gap-4">
								{message.toolInvocations.map((toolInvocation) => {
									...
									if (state === "result") {
										...
										return (
											<div key={toolCallId}>
												{toolName === "getWeather" ? <Weather weatherAtLocation={result} /> : toolName === "createDocument" ? <DocumentPreview isReadonly={isReadonly} result={result} /> : toolName === "updateDocument" ? <DocumentToolResult type="update" result={result} isReadonly={isReadonly} /> : toolName === "requestSuggestions" ? <DocumentToolResult type="request-suggestions" result={result} isReadonly={isReadonly} /> : <pre>{JSON.stringify(result, null, 2)}</pre>}
											</div>
										);
									}
									return (
										...
										>
											{toolName === "getWeather" ? <Weather /> : toolName === "createDocument" ? <DocumentPreview isReadonly={isReadonly} args={args} /> : toolName === "updateDocument" ? <DocumentToolCall type="update" args={args} isReadonly={isReadonly} /> : toolName === "requestSuggestions" ? <DocumentToolCall type="request-suggestions" args={args} isReadonly={isReadonly} /> : null}
										</div>
									);
								})}
							</div>
